...
class Discriminator(nn.Module):
    def __init__(self, channels_img, features_d):
        ...
        self.disc = nn.Sequential(
            ...
            self._block(features_d * 8, features_d * 16, 4, 2, 1), # state size (features_d * 4) * 8 * 8    
            self._block(features_d * 16, features_d * 32, 4, 2, 1), # state size (features_d * 4) * 8 * 8    
...
class Generator(nn.Module):
    def __init__(self, channels_noise, channels_img, features_g):
        ...
        self.net = nn.Sequential(
            # Input: N x channels_noise x 256 x 256
            self._block(channels_noise, features_g * 32, 4, 1, 0),  # img: 4x4
            self._block(features_g * 32, features_g * 16, 4, 2, 1),  # img: 4x4
